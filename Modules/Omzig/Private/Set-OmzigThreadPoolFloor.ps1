function Set-OmzigThreadPoolFloor {
    ...
    the ones queued for a free runspace. .NET starts with one thread per core (2 on the
    Flex instance), adds threads beyond that slowly (about 2 a second) and retires idle
    ones after ~20 seconds. So the first page after a quiet spell, which fires 10+ calls
    at once, waited seconds for threads. Measured 2026-09-23 with the anonymous
    PublicPing: 10 parallel calls after 45s idle took 5.3s, back-to-back 0.35s, and 4
    after idle 0.34s.
